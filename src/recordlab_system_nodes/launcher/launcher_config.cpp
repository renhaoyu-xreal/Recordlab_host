...
std::string expandVars(std::string text) {
  const auto default_build_dir =
      (std::filesystem::path(RECORDLAB_MASTER_SOURCE_DIR).parent_path() / "Recordlab_nodes" / "build").string();
  const std::string build_dir = getEnvOrDefault("RECORDLAB_NODES_BUILD", default_build_dir);
...
std::string defaultLauncherConfigPath() {
  return std::string(RECORDLAB_MASTER_SOURCE_DIR) + "/config/recordlab_launcher.json";
}
