...
json WatchdogNode::evaluateTarget(const json &nodes, const std::string &target) const {
  return evaluateTarget(nodes, target, json::object());
}

json WatchdogNode::evaluateTarget(const json &nodes, const std::string &target,
                                  const json &device_state) const {
  if (target.empty()) {
  ...
  for (const auto &node : nodes) {
    ...
    if (state == "alive") {
      if (device_state.is_object() && !device_state.empty()) {
        const std::string device_health = device_state.value("health", "ok");
        if (device_health == "error") {
          return {{"target", target},
                  {"health", "error"},
                  {"message", device_state.value("message", "主 agent 设备状态异常")},
                  {"device_state", device_state},
                  {"timestamp_ms", nowMs()}};
        }
        if (device_health == "stale") {
          return {{"target", target},
                  {"health", "stale"},
                  {"message", device_state.value("message", "主 agent 设备状态超时")},
                  {"device_state", device_state},
                  {"timestamp_ms", nowMs()}};
        }
        return {{"target", target},
                {"health", "ok"},
                {"message", device_state.value("message", "主 agent 在线，设备状态正常")},
                {"device_state", device_state},
                {"timestamp_ms", nowMs()}};
      }
...
json WatchdogNode::setTarget(const json &request) {
  ...
  refreshDeviceStateSubscription(node);
  auto state = evaluateTarget(client_.listNodes().value("data", json::array()), node, latestDeviceState());
  publishState(state);
...
json WatchdogNode::clearTarget(const json &) {
  ...
  refreshDeviceStateSubscription("");
...
void WatchdogNode::refreshDeviceStateSubscription(const std::string &target) {
  if (target.empty()) {
    device_state_sub_.reset();
    std::lock_guard<std::mutex> lock(device_state_mu_);
    latest_device_state_ = json::object();
    subscribed_state_topic_.clear();
    subscribed_state_endpoint_.clear();
    return;
  }

  try {
    const auto topics = client_.listTopics().value("data", json::array());
    for (const auto &topic : topics) {
      if (topic.value("node", "") != target) continue;
      if (topic.value("msg_type", "") != "recordlab_msgs/DeviceState") continue;
      const auto transport = topic.value("transport", json::object());
      if (transport.value("type", "") != "tcp_pubsub") continue;
      const std::string endpoint = transport.value("endpoint", "");
      const std::string topic_name = topic.value("topic", "");
      if (endpoint.empty() || topic_name.empty()) continue;

      {
        std::lock_guard<std::mutex> lock(device_state_mu_);
        if (subscribed_state_topic_ == topic_name && subscribed_state_endpoint_ == endpoint) return;
      }
      device_state_sub_.reset();
      {
        std::lock_guard<std::mutex> lock(device_state_mu_);
        latest_device_state_ = json::object();
        subscribed_state_topic_ = topic_name;
        subscribed_state_endpoint_ = endpoint;
      }
      device_state_sub_ = std::make_unique<Subscriber>(
          endpoint, topic_name, [this](const json &msg) {
            std::lock_guard<std::mutex> lock(device_state_mu_);
            latest_device_state_ = msg;
          });
      return;
    }
    device_state_sub_.reset();
...
json WatchdogNode::latestDeviceState() const {
  std::lock_guard<std::mutex> lock(device_state_mu_);
  return latest_device_state_;
}

void WatchdogNode::loop() {
  while (monitor_running_) {
    try {
      const std::string current_target = target();
      refreshDeviceStateSubscription(current_target);
      auto nodes = client_.listNodes().value("data", json::array());
      publishState(evaluateTarget(nodes, current_target, latestDeviceState()));
    } catch (...) {
