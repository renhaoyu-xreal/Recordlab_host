...
bool ScriptRunner::start() {
  ...
  user_log_pub_ = std::make_unique<UserLogPublisher>("/script_runner");
  ...
  user_log_pub_->registerPublisher(client_, "/script_runner");
  ...
  RL_LOG_INFO("script runner started");
  return true;
}
...
json ScriptRunner::runGoal(const json &goal, std::function<void(const json &)> feedback,
  ...
  try {
        ...
        [this, &feedback](const std::string &line) {
          if (line.rfind(kEventPrefix, 0) == 0) {
            ...
            if (type == "progress") publishProgress(event);
              ...
              publishLog(event.value("stream", "stdout"), event.value("message", ""),
                         UserLogLevel::Info, event.value("category", "script"));
            } else if (type == "user_log") {
              publishLog(event.value("stream", "stdout"), event.value("message", ""),
                         parseUserLogLevel(event.value("level", "INFO")),
                         event.value("category", "script"));
            }
            feedback(event);
            return;
          }
    ...
    result["error"] = e.what();
    publishLog("stderr", e.what(), UserLogLevel::Error, "script_runner");
    RL_LOG_ERROR(e.what());
...
void ScriptRunner::publishLog(const std::string &stream, const std::string &message,
                              UserLogLevel level, const std::string &category) {
  const auto timestamp = nowMs();
  if (log_pub_) log_pub_->publish({{"stream", stream}, {"message", message}, {"timestamp_ms", timestamp}});
  if (user_log_pub_) {
    user_log_pub_->publish(level, category, message,
                           {{"stream", stream}, {"legacy_topic", "/script_runner/log"}});
  }
  if (level == UserLogLevel::Error) RL_LOG_ERROR(message);
  else if (level == UserLogLevel::Warn) RL_LOG_WARN(message);
  else RL_LOG_INFO(message);
}
