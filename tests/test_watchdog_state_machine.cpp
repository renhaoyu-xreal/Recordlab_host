...
int main() {
    ...
    runScenario(
        "error_agent",
        [](HostMessageBus& bus, std::atomic<bool>& running) {
            replySequence(bus, running, {true, false, true});
        },
        [](HostMessageBus& bus) {
            waitForState(bus, "INITIALIZING");
            waitForState(bus, "ERROR");
            const auto reason = waitForState(bus, "ERROR", 7000);
            assert(reason == "error_check_still_succeeds");
        });

    runScenario(
        "disconnect_after_error",
        [](HostMessageBus& bus, std::atomic<bool>& running) {
            replySequence(bus, running, {true, false, false});
        },
        [](HostMessageBus& bus) {
            waitForState(bus, "INITIALIZING");
            waitForState(bus, "ERROR");
            waitForState(bus, "DISCONNECTED", 7000);
        });
