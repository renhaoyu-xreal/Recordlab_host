#!/usr/bin/env python3
"""为 Recordlab_nodes 准备项目内独立的 XREAL Qt 6.8 runtime。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project-root", help="Recordlab_nodes 工程根目录")
...
def resolve_project_root(raw_root: Optional[str]) -> Path:
    ...
    env_root = os.environ.get("RECORDLAB_NODES_ROOT", "").strip() or os.environ.get("RECORDLABC_ROOT", "").strip()
    if env_root:
    ...
    for candidate in candidates:
        ...
        if (root / "config/glasses_device_catalog.json").is_file() or (root / "CMakeLists.txt").is_file():
            return root
...
def runtime_paths(project_root: Path) -> Dict[str, Path]:
    runtime_root = project_root / "third_party" / "xreal" / "runtime"
    ...
    return {
        ...
        "wheel_path": project_root / "third_party" / "xreal" / "wheels" / "xreal_glasses-0.4.3-py3-none-any.whl",
    }
