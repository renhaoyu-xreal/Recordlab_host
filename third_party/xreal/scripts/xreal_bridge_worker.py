...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recordlab_nodes XREAL bridge worker")
    parser.add_argument("--project-root", required=True, help="Recordlab_nodes project root")
    return parser.parse_args()


def configure_python_path(project_root: Path) -> Path:
    runtime_root = Path(
        os.environ.get("RECORDLAB_XREAL_RUNTIME_ROOT", "").strip()
        or os.environ.get("RECORDLABC_XREAL_RUNTIME_ROOT", "").strip()
        or project_root / "third_party" / "xreal" / "runtime"
    )
    site_packages = Path(
        os.environ.get("RECORDLAB_XREAL_SITE_PACKAGES", "").strip()
        or os.environ.get("RECORDLABC_XREAL_SITE_PACKAGES", "").strip()
...
def ensure_va_compat_shim(project_root: Path) -> Path:
    source_path = project_root / "third_party" / "xreal" / "scripts" / "common" / "xreal_va_compat.c"
        ...
        def _on_cam_updated(self, sensor_type: Any, image_pair: Any) -> None:
            try:
                ...
                payload: dict[str, Any] = {
                    ...
                    "sensor": "rgb" if sensor_value == rgb_value else "slam",
                    "cams": [],
                }
